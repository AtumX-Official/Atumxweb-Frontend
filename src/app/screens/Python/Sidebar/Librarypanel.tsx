...
const LibraryPanel = ({
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const result: any = await window.api.file.uploadpythonlib({
        name: file.name,
...
<div className={`w-[250px] h-full rounded-md custom-scrollbar ${themeMode === "dark" ? "bg-black" : "bg-white" }`}>        
        <div className="w-full h-[35px] shrink-0 flex items-center justify-between px-4 text-black font-bold">
          <div className="flex items-center gap-2 mt-2">
            <Libraryicon className="w-5 h-5" />
            <span className={`text-lg ${themeMode === "dark" ? "text-white" : "text-black"}`}>Library</span>
            </div>
      ...
      fileNameResults.length === 0 && (
        ...
        <div className="px-3 pt-2">
          ...
          >
            ...
            <span className={`text-xs font-semibold ${themeMode === "dark" ? "text-[#561DBC]" : "text-[#722CF0]" }`}>UPLOAD</span>
          </button>
