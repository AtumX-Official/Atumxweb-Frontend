...
import PythonScaffold from './PythonScaffold'
...
import { setConnected ,setDisconnected} from '../../../../store/websocketSlice';
...
export default function PythonPage() {
  ...
  const createNewTab = (name = 'untitled', code = '', path = '', readOnly = false,source: Tab['source'] = 'user') => {
  ...
  const closeTab = async (e: React.MouseEvent, id: string) => {
    ...
    // Last remaining tab → reset it instead of removing it
    if (tabs.length === 1) {
      const untitledTab = createEmptyTab();
      setTabs([untitledTab]);
      setActiveTabId(untitledTab.id);
      return;
    }
  ...
  const appendOutput = (text: string, type: 'out' | 'err' = 'out') => {
    ...
    console.log("Output in Page1: ", output)
  };

  // --- Existing Logic Updated for Tabs ---

  useEffect(() => {
    console.log("FILE OPEN EFFECT RUNNING");
  }, [incomingFilePath, fileName]);
  useEffect(() => {
    console.log("FILE OPEN EFFECT TRIGGERED");
    if (incomingFilePath) {
      console.log("incomingFilePath", incomingFilePath);
      console.log("fileName", fileName);
      const incomingReadOnly =
        location.state?.isReadOnly ?? false;
  
      const incomingSourceType =
        location.state?.sourceType ?? 'user';
  
        window.api.file.fileOpen(incomingFilePath).then((result: any) => {
          console.log("fileOpen result", result);
        
          if (result.success) {
            const existingTab = tabs.find(
              (t) => t.path === incomingFilePath
            );
        
            console.log("existingTab", existingTab);
            console.log("BEFORE createNewTab");
            ...
            console.log("AFTER createNewTab");
          } else {
            console.log("fileOpen failed", result.error);
          }
        });
    }
  }, [incomingFilePath, fileName]);
  ...
  const handleExitAndFlash = async () => {
    ...
    try {
      // Close serial before flashing firmware
      await window.api.serial.close();
      console.log("Serial closed for firmware flash");
  ...
  useEffect(() => {
    console.log("Output in Page: ", output)
  }, [output])
    ...
    window.api.mpRemote
      ...
      .then((result) => {
        ...
        if (pythonIndex !== -1) {
          ...
          let finalBoardName = detectedName;
          ...
          if (detectedName.includes("CAYO")) {
          ...
          } else if (detectedName.includes("_SUBO_")) {
            ...
            finalBoardName = "Snowflake";
            setBoardName("Snowflake");
          }
        
          appendOutput(
            `\n> Board Detected: ${finalBoardName}\n> Port: ${boardPort}\n> Mode: ${mode}\n`,
            'out'
          );
        }
  ...
  const handleRunUsingMpRemote = async () => {
    try {
      // 1. Close serial before running with mpremote
      await window.api.serial.close();
      console.log("Serial closed for mpremote run");

      const result = await window.api.mpRemote.run(activeTab.code);
      if (result.success) {
  ...
  useEffect(() => {
    const onOutput = (_: any, data: string) => {
      ...
      console.log("Output in Page2: ", output)
    };
    ...
    const onError = (_: any, data: string) => {
      ...
      console.log("Output in Page3: ", output)
    };
      ...
      <div className="flex flex-col h-screen w-screen bg-[#722CF0] text-white">
        <PythonScaffold
          ...
          onSave={(mode: any) => {
            ...
            const success =  handlePythonSave({
              mode,
              activeTab,
              updateActiveTabData,
              appendOutput,
          ...
          onClear={() => {
            setSerialData("");
            setOutput([]);
            console.log("Output in Page clr: ", output)
          }}
        ...
        >
          <div className="flex flex-col h-full overflow-hidden">
            {/* TAB BAR */}
            <div className="relative flex items-end bg-[#722CF0] pt-2 gap-1 overflow-x-auto border-b border-black/10 ">
            <div
  className="absolute inset-0 z-10 animate-moving-bg bg-repeat bg-center bg-[length:700px]  pointer-events-none opacity-30 "
...
/>
              {tabs.map(tab => (
                <div 
                  ...
                  className={`group px-4 py-2 cursor-pointer flex items-center gap-2 rounded-t-lg z-[10] transition-all min-w-[120px] ${activeTabId === tab.id
              ...
              <button
                ...
                className="h-8 px-3 mb-1 flex items-center justify-center rounded-md bg-white text-black font-bold hover:bg-gray-100 transition-all z-10"
              >
                +
              </button>
            </div>
            ...
            <PythonEditor
              ...
              theme={theme}
              options={options}
              setContextMenuRef={setContextMenuRef}
            />
          </div>
          ...
          {showPDF && (
            <Curriculum pdfUrl={samplePdf} position={pdfPosition} onClose={() => setShowPDF(false)} title={"Python User Manual"} />
          )}
